...
export default class RenderAssignment {
	...
	constructor(obj : SnowblindComponent, options : {replace? : HTMLElement} = {}) {
	...
	Render() {
		...
		if (this.Object.hasTheme) {
			...
			const appendStyles = (elements : NodeListOf<Element> | Element[], query : string) => {
				const properties = this.Object.hasTheme[query];
				for (const subNode of (Array.from(elements) as HTMLElement[])) {
